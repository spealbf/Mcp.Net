...
namespace Mcp.Net.WebUi.Infrastructure.Notifications;
...
public class SessionNotifier
{
    ...
    public SessionNotifier(IHubContext<ChatHub> hubContext, ILogger<SessionNotifier> logger)
    {
    ...
    public async Task NotifySessionUpdatedAsync(SessionMetadataDto session)
    {
        try
        {
            _logger.LogInformation(
                "[NOTIFIER] Notifying clients about session update: {SessionId}, LastUpdatedAt: {LastUpdatedAt}",
                session.Id,
            ...
            _logger.LogInformation(
                "[NOTIFIER] Setting LastUpdatedAt to current time: {Time}",
                session.LastUpdatedAt
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "[NOTIFIER] Error notifying clients about session update: {SessionId}",
                session.Id
            );
        }
    }
    ...
    public async Task NotifySessionDeletedAsync(string sessionId)
    {
        ...
        {
            _logger.LogInformation(
                "[NOTIFIER] Notifying clients about session deletion: {SessionId}",
                sessionId
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "[NOTIFIER] Error notifying clients about session deletion: {SessionId}",
                sessionId
            );
        }
    }
